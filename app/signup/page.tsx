...
export default function SignupPage() {
  ...
  const handleOAuthSignup = (provider: string) => {
    window.location.href =
      `http://localhost:8080/oauth2/authorization/${provider}?role=${role}`;
  };


  return (
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-indigo-50 to-purple-50 dark:from-gray-900 dark:to-gray-800 p-4">
      ...
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-100 dark:border-gray-700">
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <button
            ...
            className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${role === 'PLAYER'
                ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300'
                : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:border-indigo-200'
              }`}
          >
            <User className={`w-8 h-8 mb-2 ${role === 'PLAYER' ? 'text-indigo-600' : 'text-gray-400'}`} />
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${role === 'GAMEMASTER'
                ? 'border-purple-600 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300'
                : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:border-purple-200'
              }`}
          >
            <ShieldCheck className={`w-8 h-8 mb-2 ${role === 'GAMEMASTER' ? 'text-purple-600' : 'text-gray-400'}`} />
            <span className="font-bold text-sm">GameMaster</span>
          </button>
        </div>
